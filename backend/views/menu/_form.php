...
$form = ActiveForm::begin(['id' => 'form', 'options' => ['class' => 'uk-form-stacked'], 'enableClientValidation' => false]); ?>
    ...
    <div class="uk-grid uk-grid-small uk-margin-top" uk-grid>
        ...
        <div class="uk-width-1-3@m">
            <?= $form->field($model, 'type')->dropDownList($model->types, ['prompt' => Yii::t('app', 'Выбрать тип меню'), 'class' => 'uk-select']) ?>
        </div>
        <div class="uk-width-1-3@m">
        ...
        <div class="uk-width-1-1">
            <div class="uk-grid uk-grid-small uk-child-width-1-3@m" uk-grid>
                ...
                <?php if ($model->type == 1 || $model->type == 2 || $model->type == 3): ?>
                    <div>
                        <?= $form->field($model,'application')->dropDownList(is_array($applications)?$applications:[],['prompt'=>Yii::t('app','PROMPT_SELECT'),'class'=>'uk-select'])?>
                    </div>
                <?php endif ?>

                <?php if ($model->type == 3 || $model->type == 2): ?>
                    <div>
                        <?= $form->field($model,'category')->dropDownList(is_array($categories)?$categories:[],['prompt'=>Yii::t('app','PROMPT_SELECT'),'class'=>'uk-select'])?>
                    </div>
                <?php endif ?>

                <?php if ($model->type == 3): ?>
                    <div>
                        <?= $form->field($model,'item')->dropDownList(is_array($items)?$items:[],['prompt'=>Yii::t('app','PROMPT_SELECT'),'class'=>'uk-select'])?>
                    </div>
                <?php endif ?>
            </div>
        </div>
        ...
        <div class="uk-width-1-3@m">
            <?= $form->field($model, 'parent_id')->dropDownList($model->parents, ['class' => 'uk-select', 'prompt' => Yii::t('app', 'PARENT_MENU_ITEM')]) ?>
        </div>
    </div>
    <div class="uk-margin-top">
        <?= Html::submitButton(Yii::t('app','SAVE_BUTTON'), ['class' => 'uk-button uk-button-success']) ?>
    </div>
